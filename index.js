...
function showCpuWarning(effectiveThreads) {
    ...
    console.log(chalk.gray(`\n  [i] System: ${cpus} CPU cores | ${totalMem}GB RAM (${freeMem}GB free)`));

    if (effectiveThreads >= 50000) {
        console.log(chalk.bgRed.white.bold('  +============================================================+'));
        console.log(chalk.bgRed.white.bold('  |  [!!!] EXTREME WARNING -- SYSTEM DESTRUCTION RISK          |'));
        console.log(chalk.bgRed.white.bold('  +============================================================+'));
        ...
        console.log(chalk.red.bold(`\n  [!!!] CRITICAL: ${effectiveThreads.toLocaleString()} threads -- VERY HIGH CPU/RAM usage!`));
        ...
        console.log(chalk.hex('#FF8E53').bold(`\n  [!!] WARNING: ${effectiveThreads.toLocaleString()} threads -- HIGH CPU usage expected.`));
        ...
        console.log(chalk.yellow(`\n  [!] NOTE: ${effectiveThreads.toLocaleString()} threads -- Moderate CPU usage.`));
    ...
    const { action } = await inquirer.prompt([
        {
            ...
            choices: [
                ...
                new inquirer.Separator(chalk.gray('---- [*] Defense Operations ----')),
                { name: `${chalk.hex('#43E97B')('[>]')} Defense Report Only (Recommendations)`, value: 'defense' },
...
async function acceptTerms() {
    ...
    if (!confirm) { console.log(chalk.red('\n  [x] Authorization not confirmed. Aborting.\n')); process.exit(1); }
    return true;
}
...
async function startFullAudit() {
    ...
    showPhaseHeader(3, 'DEFENSIVE ANALYSIS', '[~]');
...
async function startQuickAudit() {
    ...
    showPhaseHeader(3, 'QUICK DEFENSE REPORT', '[~]');
...
async function startDefenseOnly() {
    ...
    showPhaseHeader(1, 'DEEP RECONNAISSANCE', '[~]');
    const reconData = await runRecon(nt);
    showPhaseHeader(2, 'DEFENSIVE ANALYSIS (NO ATTACK)', '[~]');
...
async function cliMode(target, threads, time, intensityFlag) {
    ...
    console.log(chalk.white(`  Threads:     ${t} x ${intensity.multiplier}x = ${effectiveThreads} effective`));
