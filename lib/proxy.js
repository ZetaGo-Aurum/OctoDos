...
async function fetchProxies() {
    ...
    const tasks = PROXY_SOURCES.map(async (src) => {
        try {
            const resp = await axios.get(src, { timeout: 10000 });
            const lines = resp.data.split('\n').map(l => l.trim()).filter(l => /^\d+\.\d+\.\d+\.\d+:\d+$/.test(l));

            if (src.includes('socks5')) {
                lines.forEach(p => s5Set.add(p));
            } else if (src.includes('socks4')) {
                lines.forEach(p => s4Set.add(p));
            } else {
                lines.forEach(p => httpSet.add(p));
            }
