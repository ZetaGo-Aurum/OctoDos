...
After install, **`octodos`** is automatically registered as a global CLI command — run it from anywhere. If auto-register fails (e.g. permission denied on Linux), run `sudo npm link` manually.
